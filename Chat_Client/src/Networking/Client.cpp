...
{
	int result = connect(m_ClientSocket, m_ServerInfo->ai_addr, (int)m_ServerInfo->ai_addrlen);
	...
	send(m_ClientSocket, buffer.GetBufferData(), (int)buffer.GetSize(), 0);
...
void Client::HandleData()
{
	...
	int result = send(m_ClientSocket, newRequest->buffer->GetBufferData(), (int)newRequest->buffer->GetSize(), 0);
	if (result == SOCKET_ERROR) {
